...
const getColor=(n:string)=>{let h=0;for(let i=0;i<n.length;i++)h=n.charCodeAt(i)+((h<<5)-h);return COLORS[Math.abs(h)%COLORS.length]}
    ...
    <div className="flex items-center gap-3">
        <div className={`h-9 w-9 rounded-full grid place-items-center text-xs font-bold text-white ${getColor(name)}`}>{name.split(' ').map(x=>x[0]).join('').slice(0,2).toUpperCase()}</div>
